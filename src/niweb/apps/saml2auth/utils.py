...
def get_authorized_users(filename, allowed_groups=["su"]):
    def get_auth_users(groups, allowed_groups):
        if not (isinstance(allowed_groups, list) or allowed_groups == "*"):
            print(
                "Allowed groups should either be a list of group names or a string '*' for all groups"
            )
            sys.exit(1)
        auth_users = []
    ...
    def get_users_from_identifiers(
        group_identifiers, auth_users, groups, allowed_groups
    ):
    ...
    staffs = (
        config["GROUPS.STAFF"]["identifier"]
        if config.has_section("GROUPS.STAFF")
        else []
    )
    superusers = (
        config["GROUPS.SUPERUSER"]["identifier"]
        if config.has_section("GROUPS.SUPERUSER")
        else []
    )
    ...
    superusers = get_users_from_identifiers(
        superusers, auth_users, groups, allowed_groups
    )
